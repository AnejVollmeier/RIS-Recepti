...
function List() {
  ...
  const handlePorcijeChange = (receptId, value) => {
  ...
  const getPrilagojenKolicina = (recept, sestavina) => {
    ...
    if (!isNaN(stevilka)) {
      const prilagojena = (stevilka * faktor).toFixed(1);
      return kolicina.replace(stevilka.toString(), prilagojena);
    }
    return kolicina;
  };
            ...
            return (
              <div key={recept.id} className={styles.receptCard}>
                ...
                <div className={styles.porcijeContainer}>
                  <label>
                    <strong>Število porcij: {trenutnePorcije}</strong>
                  </label>
                  <div
                    style={{
                      display: "flex",
                ...
                <ul>
                  {recept.sestavine.map((sestavina, index) => (
                    <li key={index}>
                      {sestavina.naziv}:{" "}
                      {getPrilagojenKolicina(recept, sestavina)}
                    </li>
                  ))}
                </ul>
